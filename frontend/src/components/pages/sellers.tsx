...
const Button = () =>
  ...
  const setId = (id: any) => {
    ...
    console.log(id)
    location.href = "http://localhost:5173/editform?id=" + String(id)
  }
  const ButtonEdit = (id: any) => (
    <a>
      <button onClick={() => setId(id)} className="w-8 h-8 bg-blue-500 items-center justify-center rounded-full border border-white">
        ...
        <div className={`${shoes.length ? 'opacity-100 visible' : 'opacity-0 invisible'} transition-opacity duration-500`}>
          ...
          {shoes.map((shoe, i) => (
            <article key={shoe.id} className={`grid grid-cols-6 bg-[#333333] px-10 py-5 border mb-3 text-center`}>
              <div className="items-center flex justify-center gap-4"><ButtonEdit id={shoe.id}></ButtonEdit>{shoe.id}</div>
              <div>{shoe.name}</div>
